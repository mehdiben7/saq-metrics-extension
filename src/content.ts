...
function getAlcoolRateFromPage(): string {
  ...
  const alcoolElement = document.querySelector(
    'strong[data-th="Degré d\'alcool"]',
  ) as HTMLElement;
...
function getVolumeFromPage(): number {
  const volumeElement = document.querySelector(
    "div.product.attribute.format > span.value > strong.type",
  ) as HTMLElement;
  if (volumeElement) {
...
function getDisplayablePricePerLiterOfAlcohol(
  pricePerLiterOfAlcohol: number,
): string {
  return (
    convertNumberToStringWithTwoDecimals(pricePerLiterOfAlcohol) +
    " $/L d'alcool"
  );
}
...
if (isAProductPage() === true) {
  let item = new Item(
    getVolumeFromPage(),
    getRawPrice(getStringPriceFromPage()),
    getRawAlcoolRate(getAlcoolRateFromPage()),
  );

  let priceWithTax = convertNumberToStringWithTwoDecimals(
    item.getPriceWithTax(),
  );
  let pricePerLiterOfAlcohol = convertNumberToStringWithTwoDecimals(
    item.getPricePerLiterOfAlcohol(),
  );
  let pricePerLiter = convertNumberToStringWithTwoDecimals(
    item.getPricePerLiter(),
  );

  // log an error if any of the prices are NaN
  if (isNaN(item.getPriceWithTax())) {
    console.error(
      "Error: Price with tax is NaN. Please check the price format on the page.",
    );
  }

  if (isNaN(item.getPricePerLiterOfAlcohol())) {
    console.error(
      "Error: Price per liter of alcohol is NaN. Please check the alcohol rate format on the page.",
    );
  }

  if (isNaN(item.getPricePerLiter())) {
    console.error(
      "Error: Price per liter is NaN. Please check the volume format on the page.",
    );
  }
  ...
  addPricePerLiterOfAlcoholToPage(
    getDisplayablePricePerLiterOfAlcohol(item.getPricePerLiterOfAlcohol()),
  );
